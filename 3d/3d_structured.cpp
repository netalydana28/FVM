...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=15*15*15;
map<int, vector<double> > nodes;
const int center_num=2744;
map<int, vector<double> > centers;
int face_num;
int single_cell_num = 8;
...
void nodes_coor(){//checked
    string myText;
    ifstream NodesFile("coor_structured_15.txt");
    vector<double> v; 
    int cnt = 1;
    int cnt_node = 1;
    while (getline(NodesFile, myText)) {
        if(cnt%3 ==0){
            ...
            nodes[cnt_node] = v;
            v.clear();
            // cout<<nodes[cnt_node][0]<<" "<<nodes[cnt_node][1]<<" "<<nodes[cnt_node][2]<<endl;
            cnt_node++;
        }
        cnt++;
        if(cnt>node_num*3){
...
void cell_to_node(){//Checked
    string myText;
    ifstream NodesFile("links_structured_15.txt");
    vector<int> v; 
    vector<int> v1; 
    int cnt = 1;
    while (getline(NodesFile, myText)) {
        ...
        // }else{
        //     v = split_int(myText.substr(11, myText.size()-1), ' ');
        //     for(int i=7; i<9; i++){
        //         int cell = cnt/2;
        //         v1.push_back(cell);
        //         v1.push_back(i);
        //         l_cell_to_node[v1] = v[i-7];
        //         if(node_to_cell.find(v[i-7]) != node_to_cell.end()){
        //             node_to_cell[v[i-7]].push_back(cell);
        //         }else{
        //             vector<int> temp;
        //             temp.push_back(cell);
        //             node_to_cell[v[i-7]] = temp;
        //             temp.clear();
        //         }
        //         v1.clear();
        //     }
            ...
            // v1.push_back(cell);
            // v1.push_back(1);
            // cout<<"1: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(2);
            // cout<<"2: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(3);
            // cout<<"3: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(4);
            // cout<<"4: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(5);
            // cout<<"5: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(6);
            // cout<<"6: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(7);
            // cout<<"7: "<<l_cell_to_node[v1]<<endl;
            // v1.erase(v1.end()-1);
            // v1.push_back(8);
            // cout<<"8: "<<l_cell_to_node[v1]<<endl;
            // v1.clear();
        // }
        v.clear();
        cnt++;
...
void center_coordinates_generator(){//Checked without output
    ...
    for(int i = 1; i < center_num + 1; i++){
        ...
        for(int j = 1; j<single_cell_num+1; j++){
            v.push_back(j);
            register int node = l_cell_to_node[v];
            // cout<<l_cell_to_node[v]<<endl;
            x += nodes[node][0];
            y += nodes[node][1];
            z += nodes[node][2];
        ...
        // cout<<"x: "<<x<<" y: "<<y<<" z: "<<z<<endl;
        v1.push_back(x/double(single_cell_num));
        v1.push_back(y/double(single_cell_num));
        v1.push_back(z/double(single_cell_num));
...
vector<double> face_center(int face_ind){//Check changes in num node
    ...
    for(int j = 1; j<5; j++){
        v.push_back(j);
        register int node = l_face_to_node[v];
        x += nodes[node][0];
        y += nodes[node][1];
        z += nodes[node][2];
        v.erase(v.end()-1);
...
double triangle_area(vector<double> n1, vector<double> n2, vector<double> n3){
    vector <double> ans; 
    vector<double> v1;
    vector<double> v2;
    for(int i=0; i<3; i++){
...
double area(int face_ind){ 
    ...
    for(int i = 2; i < 4; i++){
        ...
        v.erase(v.end()-1); 
        v.push_back(i+1);
        int n3 = l_face_to_node[v];
        V += triangle_area(nodes[n1], nodes[n2], nodes[n3]);
        // cout<<V<<endl;
    }
...
void set_boundary_conditions(){//rewrite
    register int bound1 = 200;
    register int bound2 = 450;
...
int main(){
    ...
    //     }
    // }
    ofstream MyFile("pressure_3d_structured.dat");
    MyFile << "VARIABLES= \"X\", \"Y\", \"Z\",\"U\"" <<endl;
    for(int i=0; i<node_num; i++){
    ...
    ofstream MFile("pressure_sorted_stuctured.dat");
