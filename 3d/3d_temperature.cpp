...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=1951;
map<int, vector<double> > nodes;
const int center_num=9209;
...
void nodes_coor(){//checked
    string myText;
    ifstream NodesFile("coor_10.txt");
    vector<double> v; 
    int cnt = 1;
    int cnt_node = 1;
...
void cell_to_node(){//Checked
    string myText;
    ifstream NodesFile("links_10.txt");
    vector<int> v; 
    vector<int> v1; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
...
int main(){
    ...
    fvm();
    ofstream MyFile("temperature_3d.dat");
    MyFile << "VARIABLES= \"X\", \"Y\", \"Z\",\"T\"" <<endl;
    ...
    ofstream MFile("temperature_sorted.dat");
