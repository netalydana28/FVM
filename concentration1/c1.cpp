...
//Taking into account concentration, mesh 1x1, concentration in center
#include <bits/stdc++.h>  
using namespace std;
const int node_num=3903, center_num=7604;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400.0, dt = 1.0e-008, ro = 1.0, d = 1.0/500.0, 
    max_diff, f_volume[center_num], lim_x=1.0, right_lim = lim_x/2.0 + 0.1*lim_x, left_lim = lim_x/2.0 - 0.1*lim_x,
    source_coor = lim_x/2;
int face_num, l_cell_to_face[center_num][3], l_cell_to_node[center_num][3], source_node;
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("concent_cnt_1_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("concent_cnt_1_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
    ...
    NodesFile.close();
    cout<<" size "<<node_to_cell[source_node].size()<<endl;
    // for(int i=0; i<node_to_cell[source_node].size(); i++){
    //     // C_centers_old[node_to_cell[source_node][i]-1] = 1;
    //     cout<<i <<" "<<node_to_cell[source_node][i]<<endl;
    // }
}
...
void set_boundary_conditions_C(){
    ...
    for(int i=0; i<node_to_cell[source_node].size(); i++){
        C_centers_old[node_to_cell[source_node][i]-1] = 1;
        C_centers_new[node_to_cell[source_node][i]-1] = 1;
        // cout<<i <<" "<<node_to_cell[source_node][i]<<endl;
    }
}
