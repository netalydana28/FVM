...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=668, center_num=1254;
double nodes[node_num][2], centers[center_num][2], 
    ...
    max_diff, f_volume[center_num], lim_x=0.1, right_lim = lim_x/2.0 + 0.1*lim_x, left_lim = lim_x/2.0 - 0.1*lim_x,
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("small_grid_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("small_grid_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void fvm(){
    ...
    vector<double> face_coor;
    for(int main=0; main < 5.0e+006+1; main++){
        printf("%d \n", main);
...
int main(){
    ...
    cout<<"cells to 661 "<<node_to_cell[660].size()<<endl;
    for(i=0 ; i<node_to_cell[660].size(); i++){
        cout<<"cell "<<node_to_cell[660][i]+1<<": "<<centers[node_to_cell[660][i]][0]<<" "<<centers[node_to_cell[660][i]][1]<<endl;
    }
    set_boundary_conditions_U();
    cout<<"5"<<"\n";
    set_boundary_conditions_V();
