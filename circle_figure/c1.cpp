...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=5467, center_num=10024;
double nodes[node_num][2], centers[center_num][2], 
    ...
    max_diff, f_volume[center_num], lim_x=1.3, right_lim = lim_x/2.0 + 0.1*lim_x, left_lim = lim_x/2.0 - 0.1*lim_x,
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obst36_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obst36_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
