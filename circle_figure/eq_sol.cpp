...
double volume(int cen_ind){
    vector<int> key; 
    key.push_back(cen_ind);
    key.push_back(1);
    int n1 = l_cell_to_node[key];
    key.erase(key.end()-1);
    key.push_back(2);
    int n2 = l_cell_to_node[key];
    key.erase(key.end()-1);
    key.push_back(3);
    int n3 = l_cell_to_node[key];
...
void fvm(){
    for(int i=1; i<center_num+1; i++){
        ...
        bc -= ((-1)*sin(center_coor[0] + 2 * center_coor[1]) + 16 * exp(2 * center_coor[0] + 3 * center_coor[1])) * volume(i);
        ac -= 3*volume(i);
