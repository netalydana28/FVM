...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=2691;
map<int, vector<double> > nodes;
const int center_num=5058;
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obstacle2_coor.txt");
    vector<double> v; 
    int cnt = 1;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obstacle2_links.txt");
    vector<int> v; 
    vector<int> v1; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
...
void set_boundary_conditions(){
    ...
    for(int i=0; i<boundary_face.size(); i++){
        ...
        if(((fabs(x1-0)<=0) and (fabs(x2-0)<=0))){
