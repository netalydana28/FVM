...
void face_links(){
    ...
    for(int i=0; i<boundary_face.size(); i++){
        temp.push_back(boundary_face[i]);
        temp.push_back(1);
        nodes_boundary.push_back(l_face_to_node[temp]);
        // cout<<boundary_face[i]<<" "<<nodes_boundary[nodes_boundary.size()-1]<<endl;
    ...
    for(int i=1; i<center_num+1; i++){
        ...
        for(int j=1; j<4; j++){
            ...
            if(find(boundary_face.begin(), boundary_face.end(), face_ind) != boundary_face.end()){   
                ...
                //     bc += (area(face_ind))*P_faces[face_ind-1]*k_boundary[face_ind-1]*((nx/deltx) +(ny/delty));
                //     ac += (area(face_ind))*k_boundary[face_ind-1]*((nx/deltx) + (ny/delty));
                //     gDiff.push_back((-1)*(area(face_ind))*k_boundary[face_ind-1]*((nx/deltx) + (ny/delty)));
                // }
                // else if (deltx == 0){
                //     bc += (area(face_ind))*P_faces[face_ind-1]*k_boundary[face_ind-1]*(ny/delty);
                //     ac += (area(face_ind))*k_boundary[face_ind-1]*(ny/delty);
                //     gDiff.push_back((-1)*(area(face_ind))*k_boundary[face_ind-1]*(ny/delty));
                // }else if (delty == 0){
                //     bc += (area(face_ind))*P_faces[face_ind-1]*k_boundary[face_ind-1]*(nx/deltx);
                //     ac += (area(face_ind))*k_boundary[face_ind-1]*(nx/deltx);
                //     gDiff.push_back((-1)*(area(face_ind))*k_boundary[face_ind-1]*(nx/deltx));
                // }
                key1.clear();
            }else{
                key1.push_back(1);
                ...
                // if((deltx != 0) and (delty != 0)){
                //     gauss_seidel_coeff[i-1][out_center_ind-1] = (-1)*(area(face_ind))*k[i-1]*((nx/deltx) +(ny/delty));
                //     ac += (area(face_ind))*k[i-1]*((nx/deltx) +(ny/delty));
                //     gDiff.push_back((-1)*(area(face_ind))*k[i-1]*((nx/deltx) +(ny/delty)));
                // }else if (deltx == 0){
                //     gauss_seidel_coeff[i-1][out_center_ind-1] = (-1)*(area(face_ind))*k[i-1]*(ny/delty);
                //     ac += (area(face_ind))*k[i-1]*(ny/delty);
                //     gDiff.push_back((-1)*(area(face_ind))*k[i-1]*(ny/delty));
                // }else if (delty ==0){
                //     gauss_seidel_coeff[i-1][out_center_ind-1] = (-1)*(area(face_ind))*k[i-1]*(nx/deltx);
                //     ac += (area(face_ind))*k[i-1]*(nx/deltx);
                //     gDiff.push_back((-1)*(area(face_ind))*k[i-1]*(nx/deltx));
                // }
                key1.clear();
                // valid_denom=0;
            }
            // cout<<valid_denom<<endl;
            key.erase(key.end()-1);
            // valid_denom = 0;
        }
...
void set_boundary_conditions(){
    register int bound1 = 1;
    register int bound2 = 15;
    ...
    for(int i=bound1; i<bound2; i++){
        P_faces[boundary_face[i]-1] = 300;
    }
    vector<int> v(2); 
    for(int i=bound1; i<bound2; i++){
        v.push_back(boundary_face[i]); 
        v.push_back(1);
        P_nodes[l_face_to_node[v]-1] = 300; 
        // cout<<boundary_face[i] <<" "<<l_face_to_node[v]<<" 1"<<endl;
        v.erase(v.end()-1);
        v.push_back(2);
        P_nodes[l_face_to_node[v]-1] = 300; 
        // cout<<l_face_to_node[v]<<" 2"<<endl;
        v.clear();
    }
    // // register int bound3 = 30;
    // register int bound4 = 35;
...
int main(){
    ...
    //     temp.push_back(boundary_face[i]);
    //     temp.push_back(1);
    //     cout<<l_face_to_node[temp]<<" 1"<<endl;
    //     temp.erase(temp.end()-1);
    //     temp.push_back(2);
    //     cout<<l_face_to_node[temp]<<" 2"<<endl;
    //     temp.clear();
    // } 
    for(int i=boundary_face.size()+1; i<node_num+1; i++){
    ...
    // cout<<nodes_boundary.size()<<endl;
    ofstream MyFile("pressure_heat.dat");
