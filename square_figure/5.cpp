...
void fvm(vector<int> ind){ 
    ...
    for(int i=0; i<points.size()-1; i++){
        vector<double> temp;
        if(i == 0){
            temp.push_back(dx*(points[0][0] + points[points.size()-2][0] + points[points.size()-1][0])/3);
            temp.push_back(dx*(points[0][1] + points[points.size()-2][1] + points[points.size()-1][1])/3);
        }else{
            temp.push_back(dx*(points[i][0] + points[i-1][0] + points[points.size()-1][0])/3);
            temp.push_back(dx*(points[i][1] + points[i-1][1] + points[points.size()-1][0])/3);
        }
    ...
    for(int i=0; i<points.size(); i++){
        ...
        if(((points[i][0] - ind[0]) == 0) and ((points[i][1] - ind[1]) == 0)){
            ...
            cout<<"4: "<<P[points[i][0]][points[i][1]]<<" "<<nor[i][1]<<" "<<s[i]<<" "<<(points[i][1] - ind[1])*dx<<endl;
