//cell-centered SLE by Gauss-Seidel TRIANGLE
//to do 
// add nfaces function 
    ...
    // for(int i=center_num-1; i>=0; i--){
        ...
        for(int j=1; j<4; j++){
            ...
            if(is_boundary_face(face_ind)){
                ...
                if((deltx != 0) and (delty != 0)){
                    gauss_seidel_coeff[i][out_center_ind] = (-1)*(area(face_ind))*((nx/deltx) +(ny/delty));
                    ac += (area(face_ind))*((nx/deltx) +(ny/delty));
                    gDiff.push_back((-1)*(area(face_ind))*((nx/deltx) +(ny/delty)));
                }else if (deltx == 0){
                    gauss_seidel_coeff[i][out_center_ind] = (-1)*(area(face_ind))*(ny/delty);
                    ac += (area(face_ind))*(ny/delty);
                    gDiff.push_back((-1)*(area(face_ind))*(ny/delty));
                }else if (delty ==0){
                    gauss_seidel_coeff[i][out_center_ind] = (-1)*(area(face_ind))*(nx/deltx);
                    ac += (area(face_ind))*(nx/deltx);
                    gDiff.push_back((-1)*(area(face_ind))*(nx/deltx));
                }
                // gauss_seidel_coeff[i][out_center_ind] = (-1)*(area(face_ind))*((nx/deltx) +(ny/delty));
...
int main(){
    ...
    ifstream Cell_to_face_File("l_cell_to_face_triangle.txt");
    ...
    ifstream Face_to_cell_File("l_face_to_cell_triangle.txt");
    ...
    ifstream Face_to_node_File("l_face_to_node_triangle.txt");
    ...
    ifstream Node_to_cell_File("node_to_cell_triangle.txt"); 
    ...
    int bound1=27;
    int bound2=36;
    int bound3[9] = {9, 37, 65, 93, 121, 149, 177, 205, 233}; //left 
    // int bound1=9;
    // int bound2=19;
    // int bound3[9] = {27, 55, 83, 111, 139, 167, 195, 223, 251};//right 
    // int bound1=18;
    // int bound2=28;
    // int bound3[9] = {252, 253, 254, 255, 256, 257, 258, 259, 260};//top
    // int bound1=0;
    // int bound2=10;
