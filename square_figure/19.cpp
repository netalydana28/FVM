...
double cell_to_vertex_interpolation(int node_ind){
    ...
    double d6 = pow(pow((center6[9990] - node_coor[0]), 2) + pow((center6[1] - node_coor[1]), 2), 0.5); 
...
void gauss_seidel(){
    ...
    while(abs(to_check - find_max(P_centers))>e){
        ...
        for(int i=1; i<center_num+1; i++){
            P_centers_copy[i] = gauss_seidel_b[i-1];
            for(int j=1; j<center_num+1; j++){
                if (j == i){
                    continue;
                }
                P_centers_copy[i] = P_centers_copy[i] - (gauss_seidel_coeff[i-1][j-1] * P_centers[j]);
                P_centers[i] = (1* (P_centers_copy[i]/gauss_seidel_coeff[i-1][i-1]))+ ((1-1)*P_centers[i]);
            }
        }
    }
}
