//Среднеарифметическое + cell-to-vertex interpolation
...
int main(){
    ...
    for(int j = 0; j < 100; j++){
        for(int i = 0; i < center_num; i++){
            key_.push_back(i); 
            key_.push_back(2); 
            int face_ind1 = l_cell_to_face[key_];
            key_.erase(key_.end()-1); 
            key_.push_back(4); 
            int face_ind2 = l_cell_to_face[key_];

            key1_.push_back(face_ind1);  
            key1_.push_back(1);  
            int node_ind1 = l_face_to_node[key1_]; 
            key1_.erase(key1_.end()-1);
            key1_.push_back(2); 
            int node_ind2 = l_face_to_node[key1_]; 
            key1_.clear(); 

            key1_.push_back(face_ind2);  
            key1_.push_back(2);  
            int node_ind3 = l_face_to_node[key1_]; 
            key1_.erase(key1_.end()-1);
            key1_.push_back(1); 
            int node_ind4 = l_face_to_node[key1_]; 
            key1_.clear(); 
            key_.clear();
            cout << "node_ind " << node_ind1 << " " << node_ind2 << " " << node_ind3 << " " << node_ind4 << endl;
            P_centers[i] = (P_nodes[node_ind1] + P_nodes[node_ind2] + P_nodes[node_ind3] + P_nodes[node_ind4]) / 4;
            cout << P_nodes[node_ind1] <<" " << P_nodes[node_ind2] << " " << P_nodes[node_ind3] << " " << P_nodes[node_ind4] << endl;
            cout << P_centers[i] << endl;
        }
        cout << "^^^^^^^^^^^^^^" << endl;
        for(int i=0; i<center_num; i++){
            cout<<P_centers[i]<<" ";
