...
class interface:
    ...
    def change_frame4(self):
        if(len(self.vis_list) == 0):
            ...
            print(f"U: {self.initial_u}, V: {self.initial_v}\n")
            ...
            for i in range(len(self.vis_list)):
                if(self.vis_list[i][0] == "0"):
                    self.initial_u = np.cos((self.vis_list[i][4]-90)* np.pi/180)
                    self.initial_v = np.sin((self.vis_list[i][4]-90)* np.pi/180)
                    if(self.vis_list[i][1]== "1"):
                        self.initial_u *= (-1)
                elif(self.vis_list[i][0] == "1"): 
                    self.initial_u = np.cos((self.vis_list[i][4])* np.pi/180)
                    self.initial_v = np.sin((self.vis_list[i][4])* np.pi/180)
                    if(self.vis_list[i][1] == "1"):
                        self.initial_v *= (-1)
                length = np.sqrt(self.initial_u**2 + self.initial_v**2)
                self.initial_u /= length
                self.initial_v /= length
                txt += " ".join(self.vis_list[i][:4]) + " " + str(self.initial_u) + " " + str(self.initial_v) + " " + " ".join(self.vis_list[i][5:7])+"\n"
    ...
    def draw_P(self):
        ...
        self.plot_p = self.p.scatter(self.X_data, self.Y_data, c=self.P_data, cmap = "gist_rainbow", vmin=min(self.P_data), vmax=max(self.P_data))
    ...
    def draw_U(self):
        ...
        self.plot_u = self.u.scatter(self.X_data, self.Y_data, c=self.U_data, cmap = "gist_rainbow", vmin=min(self.U_data), vmax=max(self.U_data))
    ...
    def draw_V(self):
        ...
        self.plot_v = self.v.scatter(self.X_data, self.Y_data, c=self.V_data,cmap = "gist_rainbow", vmin=min(self.V_data), vmax=max(self.V_data))
    ...
    def draw_C(self):   
        ...
        self.plot_c = self.c.scatter(self.X_data, self.Y_data, c=self.C_data, cmap = "gist_rainbow", vmin=min(self.C_data), vmax=max(self.C_data))
    ...
    def append_data(self):
        self.side = self.sb_side.get()
        self.axis = str(self.e_axis.get())
        self.left_lim = str(self.e_left_lim.get())
        self.right_lim = str(self.e_right_lim.get())
        self.degrees = float(self.e_degrees.get())
        self.start = str(self.e_start.get())
        self.end = str(self.e_end.get())
        self.text_widget.insert(END, f"{self.side} = {self.axis}\t[{self.left_lim}, {self.right_lim}]\t\t{self.degrees}°\t[{self.start}, {self.end}]\n")
        if(self.side == "X"):
            self.side = "0"
        elif(self.side == "Y"):
            self.side = "1"
        self.vis_list.append([self.side, self.axis, self.left_lim, self.right_lim, self.degrees, self.start, self.end])
    ...
    def main_frame(self):
        ...
        set_U_sidex.grid(row=3, column=9)
        set_U_sidey.grid(row=3, column=11)
        set_U_left_lim.grid(row=4, column=9)
        set_U_right_lim.grid(row=4, column=11)
        ...
        self.text_widget.insert(END, "Side\tDistance interval\tDegree\tTime interval\n")# Insert text into the text widget
        #buttons 
