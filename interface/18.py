...
class interface:
    ...
    def main_frame(self):
        ...
        #mesh drawing
            #reading files 
            #coordinates
        f = open("C:\\Users\\Eldana\\OneDrive - АО Казахстанско-Британский Технический Университет\\Documents\\Project\\fvm\\optimization1\\mesh_coor.txt", "r")
        X_data = []
        Y_data = []
        for x in f:
            list = [float(i) for i in x.split()]
        ...
        self.coor = np.stack([self.X_data, self.Y_data]).T
            #links
        f = open("C:\\Users\\Eldana\\OneDrive - АО Казахстанско-Британский Технический Университет\\Documents\\Project\\fvm\\optimization1\\mesh_links.txt", "r")
        triangles = []
        for x in f: 
            list = x.split()
            list.pop(0)
