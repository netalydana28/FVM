...
double nodes[node_num][2], centers[center_num][2], 
    ...
    max_diff, f_volume[center_num], side, u0, v0, left_lim, right_lim;
int face_num, l_cell_to_face[center_num][3], l_cell_to_node[center_num][3], i=0, j=0, axis;
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("accurate_obst16_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("accurate_obst16_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void set_boundary_conditions(int axis, double side, double u0, double v0, double left_lim, double right_lim){
    double str_side[2], anti_side[2], lim_x = abs(side - 1);
    int b_f, n[2];
    ...
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if(axis == 0){
            str_side[0] = nodes[n[0]][0]; 
            str_side[1] = nodes[n[1]][0];
            anti_side[0] = nodes[n[0]][1]; 
            anti_side[1] = nodes[n[1]][1];
        }else if (axis == 1){
            str_side[0] = nodes[n[0]][1]; 
            str_side[1] = nodes[n[1]][1];
            anti_side[0] = nodes[n[0]][0]; 
            anti_side[1] = nodes[n[1]][0];
        }
        if((str_side[0]!=lim_x) and (str_side[1]!=lim_x)){
            U_nodes[n[0]] = 0;
            U_nodes[n[1]] = 0;
        }
    ...
    for(int l=0; l<boundary_face.size(); l++){
       ...
       if(axis == 0){
        ...
        if((str_side[0]==side) and (str_side[0]==side)){
            P_nodes[n[0]] = 0;
            P_nodes[n[1]] = 0;
        }else if((str_side[0]==lim_x) and (str_side[0]==lim_x)){
            P_nodes[n[0]] = 1;
            P_nodes[n[1]] = 1;
        }
    }
...
void fvm(int axis, double side, double left_lim, double right_lim, double u0, double v0){
    int i = 0;
    double lim_x = abs(side -1);
    get_source();
    nodes_coor();
    cout<<"1"<<"\n";
    cell_to_node();
...
int main(){
    ...
    fvm(axis, side, left_lim, right_lim, u0, v0);
    return 0;
}