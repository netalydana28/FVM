...
void set_boundary_conditions(int axis, double side, double u0, double v0, double left_lim, double right_lim){
    ...
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if(((str_side[0]==side) and (str_side[1]==side)) or ((str_side[0]!=lim_x) and (str_side[1]!=lim_x)) or ((anti_side_side[0]!=side) and (anti_side[1]!=side)) or ((anti_side[0]!=lim_x) and (anti_side[1]!=lim_x))){
            U_nodes[n[0]] = 0;
            U_nodes[n[1]] = 0;
        }
