...
double nodes[node_num][2], centers[center_num][2], 
    ...
    source_coor = lim_x/2;
int face_num, l_cell_to_face[center_num][3], l_cell_to_node[center_num][3], source_node, i=0, j=0;
...
void nodes_coor(){
    ...
    while (getline(NodesFile, myText)) {
        ...
        nodes[cnt][0] = v[0];
        nodes[cnt][1] = v[1];
        if((v[0]==v[1]) and (v[0] == source_coor)){
            source_node = cnt;
        }
        v.clear();
        cnt++;
...
void set_boundary_conditions_C(){
    ...
    C_nodes[source_node] = 1;
    for(i=0; i<node_to_cell[source_node].size(); i++){
        C_centers_old[node_to_cell[source_node][i]] = 1;
        C_centers_new[node_to_cell[source_node][i]] = 1;
        C_nodes[l_cell_to_node[node_to_cell[source_node][i]][0]] = 1;
        C_nodes[l_cell_to_node[node_to_cell[source_node][i]][1]] = 1;
        C_nodes[l_cell_to_node[node_to_cell[source_node][i]][2]] = 1;
    }
}
        ...
        for(i=0; i<center_num; i++){
            ...
            for(j=0; j<3; j++){
                ...
                if(is_boundary_face(face_ind)){   
                    ...
                    ac += face_areas[face_ind]*(C_faces[face_ind] - C_centers_old[i])*((d*valid_denom) - U_centers_old[i]*nx - V_centers_old[i]*ny);
                }else{
                    nx = sn[face_ind][0];
            ...
            for(i=0; i<center_num; i++){
                ...
                for(j=0; j<3; j++){
                    ...
                    if(is_boundary_face(face_ind)){   
                        ...
                        nom += face_areas[face_ind]*P_faces[face_ind]*valid_denom;
        ...
        for(i=0; i<center_num; i++){
            ...
            for(j=0; j<3; j++){
                ...
                if(is_boundary_face(face_ind)){   
                    ...
                    au += face_areas[face_ind]*(P_faces[face_ind] - P_centers_new[i])*nx;
                    av += face_areas[face_ind]*(P_faces[face_ind] - P_centers_new[i])*ny;
                }else{
                    nx = sn[face_ind][0];
                    out_center_ind = l_face_to_cell[face_ind][0]; 
                    c2[0] = centers[out_center_ind][0];
