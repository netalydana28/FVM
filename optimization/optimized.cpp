...
#include <string>
using namespace std;
const int node_num=13060, center_num=25406;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400.0, dt = 1.0e-006, ro = 1.0, d = 1.0/800.0, 
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obst16_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obst16_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
