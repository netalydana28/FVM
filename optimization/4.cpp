...
//Taking into account concentration, mesh 1x1, 16 obstacles, starting from file on certain iteration 
#include <bits/stdc++.h>  
using namespace std;
...
void read_prev_results(){
    string myText;
    ifstream NodesFile("out10000.dat");
    vector<double> v; 
    int cnt = 0;
    while (getline(NodesFile, myText)) {
        v = split_double(myText, ' ');
...
int is_boundary_face(int face_ind){
    ...
    for(int main=0; main < 5.0e+006; main++){
        ...
        for(i=0; i<center_num; i++){
            ...
            for(j=0; j<3; j++){
                ...
                switch(is_boundary_face(face_ind)){ 
                    case 1:  
                        face_coor = f_centers[face_ind];
