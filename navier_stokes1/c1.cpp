//Just concentration solving
#include <bits/stdc++.h>  
using namespace std;
const int node_num=4015, center_num=7828;
double nodes[node_num][2], centers[center_num][2], 
    C_nodes[node_num],
    C_centers_old[center_num], C_centers_new[center_num], 
    eps=0.0001, dt = 0.00001, ro = 1.0, d = 0.01, max_diff, f_volume[center_num];
...
vector<double> face_areas, C_faces;
...
void cell_to_vertex_interpolation_old(){ 
    for(int node_ind=1; node_ind<node_num+1; node_ind++){
        double node_coor[2], center_coor[2], nom_C = 0, denom = 0;  
        ...
        for(int i=0; i<node_to_cell[node_ind-1].size(); i++){ 
            ...
            double C = C_centers_old[cell_ind - 1];  
            nom_C += C/d; 
            denom += 1/d; 
        } 
...
void cell_to_vertex_interpolation_new(){ 
    for(int node_ind=1; node_ind<node_num+1; node_ind++){
        ...
        for(int i=0; i<node_to_cell[node_ind-1].size(); i++){ 
            ...
            center_coor[1] = centers[cell_ind-1][1]; 
            double d = pow(pow((center_coor[0] - node_coor[0]), 2) + pow((center_coor[1] - node_coor[1]), 2), 0.5); 
            double C = C_centers_new[cell_ind - 1]; 
...
void fvm(){
    for(int main=0; main<1.0e+006; main++){
        ...
        for(int i=1; i<center_num+1; i++){
            ...
            for(int j=1; j<4; j++){
                ...
                if(is_boundary_face(face_ind)){   
                    ...
                    ac += face_areas[face_ind-1]*(C_faces[face_ind-1] - C_centers_old[i-1])*valid_denom;
                }else{
                    nx = sn[face_ind-1][0];
                    ...
                    ac += face_areas[face_ind-1]*valid_denom*(C_centers_old[out_center_ind-1]-C_centers_old[i-1]);
                }
            }
            C_centers_new[i-1] = C_centers_old[i-1] + ((dt*d)/f_volume[i-1]) * ac;
        }
        for(int i = 0; i< center_num; i++){
        ...
        if(main == 3.0e+005){
            ofstream yFile10("out300K.dat");
            yFile10 << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
            for(int i = 0; i<node_num; i++){
                yFile10 << nodes[i][0] << " " << nodes[i][1] <<" "<<C_nodes[i]<<"\n";
            }
            yFile10.close();
        }
        if(main == 2.0e+005){
            ofstream yFile100("out200K.dat");
            yFile100  << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
            for(int i = 0; i<node_num; i++){
                yFile100 << nodes[i][0] << " " << nodes[i][1] <<" " <<C_nodes[i]<<"\n";
            }
            yFile100.close();
        }
        if(main == 5.0e+005){
            ofstream yFile500("out500K.dat");
            yFile500  << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
            for(int i = 0; i<node_num; i++){
                yFile500 << nodes[i][0] << " " << nodes[i][1] <<" " <<C_nodes[i]<<"\n";
            }
            yFile500.close();
        }
        if(main == 7.0e+005){
            ofstream yFile1000("out700K.dat");
            yFile1000 << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
            for(int i = 0; i<node_num; i++){
                yFile1000 << nodes[i][0] <<" " << nodes[i][1] << " " <<C_nodes[i]<<"\n";
            }
            yFile1000.close();
        }
        if(main == 8.0e+005){
            ofstream yFile50000("out800K.dat");
            yFile50000  << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
            for(int i = 0; i<node_num; i++){
                yFile50000 << nodes[i][0] <<" " << nodes[i][1] << " " <<C_nodes[i]<<"\n";
            }
            yFile50000.close();
        }
    }
    ofstream yFile("out1M.dat");
    yFile << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
    for(int i = 0; i<node_num; i++){
        yFile << nodes[i][0] << " " << nodes[i][1] << " " <<C_nodes[i]<<"\n";
    }
    yFile.close();
}
int main(){
