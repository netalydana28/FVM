...
//Taking into account concentration, 16 obstacles, 0.9x0.9 mesh
#include <bits/stdc++.h>  
using namespace std;
const int node_num=3156, center_num=5782;
double maxerr = 99999, temperr;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400, dt = 0.0000001, ro = 1.0, d = 1.0/500.0, max_diff, f_volume[center_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obst16_2_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obst16_2_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void set_boundary_conditions_C(){
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if((x1==0.0) and (x2==0.0) and (y1>=0.36) and (y1<=0.54) and (y2>=0.36) and (y2<=0.54)){
            C_nodes[n[0]-1] = 1;
            C_nodes[n[1]-1] = 1;
        }
...
void set_boundary_conditions_P(){
    vector<int> v;
    //double nx, ny, area, dx, dy, x1, x2; int face_ind, fn1, n[2], b_f;
    for(int l=0; l<boundary_face.size(); l++){
    ...
    for(int main=0; main < 5.0e+006; main++){
        ...
        do{
            ...
            for(int i = 0; i< center_num; i++){
                ...
                temperr = max(temperr, fabs(P_centers_new[i] - P_centers_old[i]));
