...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=2362, center_num=4444;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400.0, dt = 0.000001, ro = 1.0, d = 1.0/500.0, max_diff, f_volume[center_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obstacle1_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obstacle1_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
