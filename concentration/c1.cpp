...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=2863, center_num=5392;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 8.8888e-008, dt = 1.0e-005, ro = 1.0, d = 8.8888e-008, max_diff, f_volume[center_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obstacle4_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obstacle4_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void set_boundary_conditions_C(){
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if(((x1==0.0) and (x2==0.0)) or ((y1==0.0) and (y2==0.0)) or ((y1==2.5) and (y2==2.5))){
            C_nodes[n[0]-1] = 0;
            C_nodes[n[1]-1] = 0;
        }else if(x1==x2){
            for(int k = 0; k<2; k++){
        ...
        if((x1==0.0) and (x2==0.0) and (y1>=1.0) and (y1<=1.5) and (y2>=1.0) and (y2<=1.5)){
            C_nodes[n[0]-1] = 1;
            C_nodes[n[1]-1] = 1;
        }
