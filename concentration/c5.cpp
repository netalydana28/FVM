...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=5551, center_num=10520;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/8.0e+005, dt = 0.00001, ro = 1.0, d = 1.0/8.0e+005, max_diff, f_volume[center_num];
...
void set_boundary_conditions_C(){
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if(((x1==1.0) and (x2==1.0)) or ((y1==1.0) and (y2==1.0)) or ((y1==4.0) and (y2==4.0))){
            C_nodes[n[0]-1] = 0;
            C_nodes[n[1]-1] = 0;
        }else if(x1==x2){
            for(int k = 0; k<2; k++){
        ...
        if((x1==0.0) and (x2==0.0) and (y1>=2.2) and (y1<=2.8) and (y2>=2.2) and (y2<=2.8)){
            C_nodes[n[0]-1] = 1;
            C_nodes[n[1]-1] = 1;
        }
...
void set_boundary_conditions_V(){
    for(int l=0; l<boundary_face.size(); l++){
        ...
        n[1] = l_face_to_node[b_f-1][1];
        double x2 = nodes[n[1]-1][0], y2 = nodes[n[1]-1][1]; 
        if(((x1==1.0) and (x2==1.0)) or ((y1==1.0) and (y2==1.0)) or ((y1==7.0) and (y2==7.0))){
            V_nodes[n[0]-1] = 0;
            V_nodes[n[1]-1] = 0;
        }else if((x1==4.0) and (x2==4.0)){
            for(int k = 0; k<2; k++){
                for(int i=0; i<node_to_cell[n[k]-1].size(); i++){
                    double center_coor[2], nom = 0, denom = 0, valid_denom;
                    center_coor[0] = centers[node_to_cell[n[k]-1][i]-1][0];
...
int main(){
    ...
    cout<<"3 "<<centers[5000][0]<<"\n";
    face_links();
    cout<<"4"<<"\n";
    cout<<"Bound "<<boundary_face.size()<<" FAce num "<< face_num<<endl;
    vector<double> v1;
    vector<double> v2;
