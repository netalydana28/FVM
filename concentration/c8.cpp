...
//Taking into account concentration, 9 obstacles, mesh 1.4X1.4
#include <bits/stdc++.h>  
using namespace std;
const int node_num=6304, center_num=11984;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400000.0, dt = 0.00001, ro = 1.0, d = 1.0/800000.0, max_diff, f_volume[center_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obstacle9_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obstacle9_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void face_links(){
    ...
    for(int i=1; i<center_num+1; i++){
        for(int j=0; j<3; j++){
            register int trg_node = l_cell_to_node[i-1][j];
...
void set_boundary_conditions_C(){
    for(int l=0; l<boundary_face.size(); l++){
        ...
        if((x1==0.0) and (x2==0.0) and (y1>=0.66) and (y1<=0.84) and (y2>=0.66) and (y2<=0.84)){
            C_nodes[n[0]-1] = 1;
            C_nodes[n[1]-1] = 1;
        }
