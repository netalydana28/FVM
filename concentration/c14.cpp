...
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 1.0/400.0, dt = 4.0e-006, ro = 1.0, d = 1.0/500.0, max_diff, f_volume[center_num];
...
void fvm(){
    for(int main=0; main < 5.0e+006; main++){
        ...
        cell_to_vertex_interpolation_new();
        set_boundary_conditions_P();
        // double max_P=-9999999999, min_P=9999999; 
        ...
        for(int i=1; i<center_num+1; i++){
            ...
            // max_P = max(max_P,  P_centers_new[i-1]);
            // min_P = min(min_P,  P_centers_new[i-1]);
