...
#include <bits/stdc++.h>  
using namespace std;
const int node_num=2882, center_num=5430;
double nodes[node_num][2], centers[center_num][2], 
    ...
    eps=0.0001, nyu = 6.0e-088, dt = 1.0e-05, ro = 1.0, d = 6.0e-088, max_diff, f_volume[center_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("obstacle_small_coor.txt");
    vector<double> v; 
    int cnt = 0;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("obstacle_small_links.txt");
    vector<int> v; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
        int cell = v[0];
...
void fvm(){
    for(int main=0; main < 1.0e+006; main++){
        ...
        //     // }
        //     // yFile50000.close();
        //     ofstream yFile50000("out10K_centers.dat");
        //     yFile50000  << "VARIABLES= \"X\", \"Y\", \"C\"" <<"\n";
        //     for(int i = 0; i<center_num; i++){
        //         yFile50000 << centers[i][0] << " " << centers[i][1]<<" "<<C_centers_old[i]<<"\n";
        //     }
        //     yFile50000.close();
        // }
    }
