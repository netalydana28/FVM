...
double nodes[node_num][2], centers[center_num][2], 
    P_nodes[node_num], U_nodes[node_num], V_nodes[node_num], 
    P_centers_old[center_num], P_centers_new[center_num], 
    U_centers_old[center_num], U_centers_new[center_num], 
    V_centers_old[center_num], V_centers_new[center_num], f[center_num], 
    eps=0.0001, d = 1.0/200.0, dt = 0.00001, ro = 1.0, max_diff, f_volume[center_num];
...
vector<double> face_areas, P_faces, U_faces, V_faces;
...
void cell_to_vertex_interpolation_old(){ 
    for(int node_ind=1; node_ind<node_num+1; node_ind++){
        double node_coor[2], center_coor[2], nom_U = 0, denom = 0, nom_V = 0, nom_P = 0;  
        ...
        for(int i=0; i<node_to_cell[node_ind-1].size(); i++){ 
            ...
            double P = P_centers_old[cell_ind - 1],  U = U_centers_old[cell_ind - 1], V = V_centers_old[cell_ind - 1]; 
            nom_P += P/d;
            nom_U += U/d; 
            nom_V += V/d; 
            denom += 1/d; 
...
void cell_to_vertex_interpolation_new(){ 
    for(int node_ind=1; node_ind<node_num+1; node_ind++){
        ...
        for(int i=0; i<node_to_cell[node_ind-1].size(); i++){ 
            ...
            center_coor[1] = centers[cell_ind-1][1]; 
            double d = pow(pow((center_coor[0] - node_coor[0]), 2) + pow((center_coor[1] - node_coor[1]), 2), 0.5); 
            double P = P_centers_new[cell_ind - 1], U = U_centers_new[cell_ind - 1], V = V_centers_new[cell_ind - 1]; 
            nom_P += P/d;
            nom_U += U/d; 
            nom_V += V/d; 
            denom += 1/d; 
...
void set_boundary_conditions_P(){
    ...
    for(int l=0; l<boundary_face.size(); l++){
        ...
        }else if (((y1==1.0) and (y2==1.0)) or ((y1==0.0) and (y2==0.0))){
            for(int k = 0; k<2; k++){
                for(int i=0; i<node_to_cell[n[k]-1].size(); i++){
                    double center_coor[2], nom = 0, denom = 0, valid_denom;
                    center_coor[0] = centers[node_to_cell[n[k]-1][i]-1][0];
...
void fvm(){
    for(int main=0; main < 1.0e+006; main++){
        ...
        // if(main == 2.5e+005){
        //     ofstream yFile1000("out250K.dat");
        ...
        // }
        // if(main == 5.0e+005){
        //     ofstream yFile50000("out500K.dat");
