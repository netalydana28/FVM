        ...
        if(((x1==0.0) and (x2==0.0)) or ((y1==0.0) and (y2==0.0)) or ((y1==1.0) and (y2==1.0))){
            ...
            for(int k = 0; k<2; k++){
                for(int i=0; i<node_to_cell[n[k]].size(); i++){
                    ...
                    double center_coor[2]; 
                    center_coor[0] = centers[node_to_cell[n[k]][i]-1][0];
                    center_coor[1] = centers[node_to_cell[n[k]][i]-1][1];
                    double nom = 0, denom = 0, valid_denom;
                    for(int j=1; j<4; j++){
                        v.push_back(j);
                        // cout<<"key "<<v[0]<<" "<<v[1]<<"\n";
                        int face_ind = l_cell_to_face[node_to_cell[n[k]][i]-1][j-1];
                        v.erase(v.end()-1);
