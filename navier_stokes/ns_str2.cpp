...
void u_star(){
    for(int i =0; i<node_num; i++){
        ...
        }else if ((fabs(x-left_boundary)==0) or (fabs(y-bottom_boundary)==0) or (fabs(y-up_boundary)==0)){
            u_star_nodes[i] = 0;
        }else{
...
void P_find(){ 
    ...
    for(int i=0; i<node_num; i++){ 
        ...
        double x = nodes[i+1][0];
        double y = nodes[i+1][1];
        if((fabs(x-right_boundary) ==0) or (fabs(y-up_boundary) == 0) or (fabs(y-bottom_boundary) == 0)){
            cnt_u *= 2;
            cnt_v *= 2;
            cnt_u -= 2*u_star_nodes[i];
            cnt_v -= 2*v_star_nodes[i];
        }else{
...
void uv_find(){
    ...
    for(int i=0; i<node_num; i++){
        ...
        while(s_it != l_node_to_nodes[i].end()){ 
            ...
            if((nodes[*s_it][0] - nodes[i+1][0]) >= 0.02){
                // cout <<  nodes[*s_it][0] - nodes[i+1][0] << endl;
                if(fabs(nodes[*s_it][0] - right_boundary) == 0){
                    cnt_u = 0;
                }
                else{
            ...
            if((nodes[*s_it][1] - nodes[i+1][1]) >= 0.02){
                // cout << nodes[*s_it][1] - nodes[i+1][1] << endl;
                if(fabs(nodes[*s_it][1] - up_boundary) == 0){
                    cnt_v = 0;
                }else if(fabs(nodes[i+1][1] - bottom_boundary) == 0){
                    cnt_v = 0;
                }else{
        ...
        //     // }
        //     dP_dx = 0;
        // }else if(fabs(y-bottom_boundary) == 0 or fabs(y-up_boundary) == 0){
        //     // int i_1;
        //     // set <int>::iterator s_it = l_node_to_nodes[i].begin(); 
        //     // while(s_it != l_node_to_nodes[i].end()){ 
        //     //     if((nodes[*s_it][1] - nodes[i+1][1]) >= 0.02){
