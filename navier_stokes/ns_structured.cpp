...
void u_star(){
    for(int i =0; i<node_num; i++){
        ...
        }else if (((fabs(x-left_boundary)==0) or (fabs(x-right_boundary)==0)) and ((fabs(y-bottom_boundary)==0) or (fabs(y-up_boundary)==0))){
            u_star_nodes[i] = 0;
        }else{
...
void P_find(){ 
    ...
    ofstream UyFile("uv_derivative.dat");
    UyFile << "VARIABLES= \"X\", \"Y\", \"K\"" <<endl;
    for(int i=0; i<node_num; i++){
        UyFile << nodes[i+1][0] << " " << nodes[i+1][1] <<" "<<P_nodes[i] <<endl;
    }
    UyFile.close();
    double to_check = 1;
    double deltx = 0.02;
...
void uv_find(){
    ...
    UyFile << "VARIABLES= \"X\", \"Y\", \"P\", \"U\", \"V\"" <<endl;
    for(int i=0; i<node_num; i++){
...
//     //Poisson equation 
//     for(int i=0; i<center_num; i++){ 
//         double deltx = 0.183673469;  
//         double cnt_u = 0; 
//         double cnt_v = 0; 
//         set <int>::iterator s_it = l_center_to_centers[i].begin(); 
//         cout<<"uij "<<i+1<<" ";
//         while(s_it != l_center_to_centers[i].end()){ 
//             if((centers[*s_it][0] - centers[i+1][0]) > 0){ 
//                 cnt_u += u_star_centers[*s_it-1];  
//                 cout<<"ui+1j "<<*s_it<<" ";
//             }
//             if((centers[*s_it][1] > centers[i+1][1]) > 0){ 
//                 cnt_v += v_star_centers[*s_it-1];    
//                 cout<<"uij+1 "<<*s_it<<" ";
//             }
//             s_it++;
//         } 
...
//         cnt_u -= (u_star_centers[i]); 
//         cnt_v -= (v_star_centers[i]); 

//         u_star_centers[i] = cnt_u/deltx; 
//         v_star_centers[i] = cnt_v/deltx; 
//         // P_centers[i] = (ro/delt) * (cnt); 
//     } 
//     for(int i=0; i<center_num; i++){
...
//     UyFile << "VARIABLES= \"X\", \"Y\", \"K\"" <<endl;
//     for(int i=0; i<center_num; i++){
//         UyFile << centers[i+1][0] << " " << centers[i+1][1] <<" "<<P_centers[i] <<endl;
//     }
//     UyFile.close();
...
void neighbour_centers(int k){ 
    // vector<int> nodes_ind; 
    set <int>::iterator s_it1 = l_center_to_centers[k].begin(); 
    while(s_it1 != l_center_to_centers[k].end()){ 
        cout << (*s_it1) << " " << centers[*s_it1][0] << " " << centers[*s_it1][1] << endl; 
        s_it1++; 
...
int main(){
    ...
    // neighbour_centers(0);
    // neighbour_centers(4);
    // neighbour_centers(10);
    // neighbour_centers(55);
