...
double coeff[node_num-2][node_num-2];
double b[node_num-2];
double P[node_num-2]; 
double P_copy[node_num-2];
double u = 1;
double v = 0;
double d = 0.1;
double delt = 0.5;
...
void gauss_seidel_nodes(){ 
    ...
    double to_check = 1; 
    while(abs(to_check - find_max(P))>e){ 
        to_check = find_max(P); 
        for(int i=0; i<node_num-2; i++){ 
            ...
            for(int j=0; j<node_num-2; j++){ 
                ...
                P_copy[i] = P_copy[i] - (coeff[i][j] * P[j]); 
                P[i] = (P_copy[i]/coeff[i][i]); 
            } 
        } 
    } 
}
...
void fdm_poisson(){  
    ...
    for(int i=1; i<node_num+1; i++){  
        if(l_node_to_nodes[i-1].size() != 4){  
            continue;  
        }  
        set <int>::iterator s_it = l_node_to_nodes[i-1].begin();  
        // cout<<"Here"<<endl;  
        int row = (i/10)%10;  
        int col = i%10;  
        // cout<<"i "<<i<<" row "<<row<<" col "<<col<<endl; 
        b[row-1] = 0;  
        while(s_it != l_node_to_nodes[i-1].end()){  
            // deltx = fabs(nodes[i+1][0] - nodes[*s_it][0]);  
            // cout<<"nodes "<<i+1<<" "<<*s_it<<endl;  
            if(l_node_to_nodes[i-1].size() == 4){  
                // cout<<"here"<<endl;  
                int col1 = *(s_it) % 10;  
                coeff[row-1][col1-1] = 1/pow(deltx, 2);  
            }else{  
                b[row-1] -= P_nodes[*(s_it) - 1];  
                // coeff[i][*(s_it)-1] = 0;  
            }  
            s_it++;  
        }  
        // cout<< -(4/(pow(deltx, 2)))<<endl; 
        // cout<<deltx<<endl;  
        coeff[row-1][col-1] = -(4/(pow(deltx, 2)));  
        b[row-1] -= P_nodes[i-1];  
    }  
    gauss_seidel_nodes();  
}
...
int main(){
    ...
    //     v.push_back(1);
    //     int node1 = l_face_to_node[v];
    //     v.erase(v.end()-1);
    //     v.push_back(2);
    //     int node2 = l_face_to_node[v];
    //     v.clear();
    ...
    for(int i=0; i<node_num-2; i++){
        ...
        MFile << nodes[i+1][0] << " " << nodes[i+1][1] <<" "<<P[i]<<endl;
    }
    MFile.close();
