...
int main(){
    ...
    for(int k=0; k<10; k++){
        ...
        for(j = 0; j<m; j++){
            // l1[0][j] = 0;
            // l2[0][j] = 0; 
            l1[n-1][j] = (l1[n-2][j] - l1[n-1][j])/dx; 
            l2[n-1][j] = (l2[n-2][j] - l2[n-1][j])/dy;
        }
        // for(i=0; i<n; i++){
        //     l1[i][0] = 0;
            // l2[i][0] = 0;
        ...
        for(j = 0; j<m; j++){
            u_star[0][j] = 0;
            v_star[0][j] = 0; 
            u_star[n-1][j] = (u_star[n-2][j] - u_star[n-1][j])/dx; 
            v_star[n-1][j] = (v_star[n-2][j] - v_star[n-1][j])/dy;
        }
        for(i=0; i<n; i++){
            u_star[i][0] = 0;
            v_star[i][0] = 0;
        ...
        for(j = 0; j<m; j++){
            oldP[0][j] = 0;
            newP[0][j] = 0; 
            oldP[n-1][j] = (oldP[n-2][j] - oldP[n-1][j])/dx; 
            newP[n-1][j] = (newP[n-2][j] - newP[n-1][j])/dx;
        }
        for(i=0; i<n; i++){
            oldP[i][0] = (oldP[i][1] - oldP[i][0])/dy;
            newP[i][0] = (newP[i][1] - newP[i][0])/dy;
            oldP[i][m-1] = (oldP[i][m-2] - oldP[i][m-1])/dy;
            newP[i][m-1] = (newP[i][m-2] - newP[i][m-1])/dy;
        }
        for(j = 0; j<m; j++){
            newU[0][j] = 0;
            newV[0][j] = 0; 
            newU[n-1][j] = (newU[n-2][j] - newU[n-1][j])/dx; 
            newV[n-1][j] = (newV[n-2][j] - newV[n-1][j])/dx;
        }
        for(i=0; i<n; i++){
            newU[i][0] = 0;
            newV[i][0] = 0;
