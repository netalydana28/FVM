#include <bits/stdc++.h>  
using namespace std;
const int node_num=2500;
map<int, vector<double> > nodes;
const int center_num=2401;
...
double u_nodes[node_num];
double u_nodes_copy[node_num];
double v_nodes[node_num];
double v_nodes_copy[node_num];
...
void nodes_coor(){
    string myText;
    ifstream NodesFile("str_coor.txt");
    vector<double> v; 
    int cnt = 1;
...
void cell_to_node(){
    string myText;
    ifstream NodesFile("str_links.txt");
    vector<int> v; 
    vector<int> v1; 
    while (getline(NodesFile, myText)) {
        v = split_int(myText, ' ');
    ...
    for(int j=0; j<3; j++){
        ...
        for(int i=0; i<node_num; i++){
            ...
            while(s_it != l_node_to_nodes[i].end()){
                ...
                // cout << "node " << *s_it << " P " << P_nodes_copy[*s_it-1] << endl;
                s_it++;
            }
            // cout << "cnt " << cnt << endl;
...
void set_boundary_conditions_P(){
    ...
    for(int i=0; i<boundary_face.size(); i++){
        ...
        double x2 = nodes[l_face_to_node[v]][0];
        double y2 = nodes[l_face_to_node[v]][1]; 
        if((fabs(x1-1)<=0) and (fabs(x2-1)<=0) and (y1>1) and (y1<10) and (y2>1) and (y2<10)){
        // if((fabs(x1-1)<=0) and (fabs(x2-1)<=0)){
            nodes_boundary.push_back(n1);
            nodes_boundary.push_back(n2);
            P_nodes[n1-1] = 1;
...
//                 continue;
//             }
//             u_star_nodes[i] = (u_star_nodes[i+1] - u_star_nodes[i])/0.183673;
//             v_star_nodes[i] = (v_star_nodes[i+1] - v_star_nodes[i])/0.183673;
//             cout<<i<<" u: "<<u_star_nodes[i]<<" "<<u_star_nodes[i+1]<<endl;
//             cout<<"   v: "<<v_star_nodes[i]<<" "<<v_star_nodes[i+1]<<endl;


//         }
//     // }
//     for(int i=0; i<node_num; i++){
//         P_nodes[i] = (ro/delt) * (u_star_nodes[i] + v_star_nodes[i]);
//         // P_nodes[i] = (u_star_nodes[i] + v_star_nodes[i]);
//         if(l_node_to_nodes[i].size() != 4){
...
void P_find(){
    ...
    for(int j = 0; j < 100; j++){
        for(int i = 0; i < node_num; i++){
            ...
            P_nodes[i] = (-ro * deltx) / (4 * delt) * (u_star_nodes[i+1] - u_star_nodes[i] + v_star_nodes[i+1] - v_star_nodes[i]) + cnt / 4;
            cnt = 0;
        }
    }
}
...
int main(){
    ...
    cout<<"5"<<endl;
    // for(int i=0; i<node_num; i++){
    //     cout << nodes[i+1][0] << " " << nodes[i+1][1] <<" "<<u_nodes[i]<<endl;
    // }
    ...
    // for(int i=0; i<node_num; i++){
    //     cout << nodes[i+1][0] << " " << nodes[i+1][1] <<" "<<l1_nodes[i]<<endl;
    // }
